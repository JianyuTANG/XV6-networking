...
//
static void
ping(uint16_t sport, uint16_t dport, int attempts)
{
  int fd;
...
dns_rep(uint8_t *ibuf, int cc)
{
  ...
  if(!record) {
    cprintf("Didn't receive an arecord\n");
    exit(1);
  }
}
...
{
  int i, ret;
  uint16_t dport = NET_TESTS_PORT;

  cprintf("nettests running on port %d\n", dport);

  cprintf("testing one ping: ");
  cping(2000, dport, 2);
  cprintf("OK\n");

  cprintf("testing single-process pings: ");
  for (i = 0; i < 100; i++)
    ping(2000, dport, 1);
  cprintf("OK\n");

  cprintf("testing multi-process pings: ");
  for (i = 0; i < 10; i++){
    int pid = fork();
    if (pid == 0){
